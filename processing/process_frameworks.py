...
def store_embeddings_in_firestore(
    ...
    for idx, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
        ...
        page_num = (
            ...
            else (chunk.get("pages", [1])[0] if chunk.get("pages") else 1)
        )
...
def process_framework_pdf(gcs_path: str, framework_name: str) -> None:
    ...
    batch_size = (
        20  # Reduced to avoid token limit (20 * 500 = ~10000 tokens, well under 20000)
    )
    all_embeddings = []
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        print(
            f"Processing batch {i // batch_size + 1}/{(len(chunks) + batch_size - 1) // batch_size} ({len(batch_texts)} chunks)..."
        )
        batch_embeddings = generate_text_embeddings(
            batch_texts, task_type="RETRIEVAL_DOCUMENT", output_dimensionality=768
        )
...
def main():
    ...
    frameworks = [
        ...
        {"name": "ISO27001", "gcs_path": "frameworks/ISO27001_ExOv.pdf"},
        {"name": "ISO42001", "gcs_path": "frameworks/ISO-IEC-42001-2023.pdf"},
        {"name": "NIST", "gcs_path": "frameworks/NIST.CSWP.29.pdf"},
        {
