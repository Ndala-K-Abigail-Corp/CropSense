 ...
 */
function chunkText(text, pageNumber, maxChars = 512, overlapChars = 50) {
    const chunks = [];
 ...
 */
async function generateEmbeddings(texts, projectId, location = 'us-east1') {
    ...
    const embeddings = response.predictions?.map((pred) => pred.embeddings.values) || [];
    return embeddings;
}
/**
 * Update document processing status
 */
async function updateDocumentStatus(documentId, status, metadata, errorMessage) {
 ...
 */
async function isDocumentProcessed(documentId) {
    const statusRef = db.collection(DOCUMENT_STATUS_COLLECTION).doc(documentId);
    const doc = await statusRef.get();
    if (!doc.exists) {
        return false;
...
export const processNewDocument = functions
    ...
    try {
        ...
        const documentId = filePath?.split('/').pop()?.replace(/\.[^/.]+$/, '') || 'unknown';
        await updateDocumentStatus(documentId, 'failed', undefined, error instanceof Error ? error.message : String(error));
        // Don't throw error to prevent function retries
        return null;
    }
});
//# sourceMappingURL=processNewDocument.js.map